package competion.google.foobar;

/**
 * There's some unrest in the minion ranks: minions with ID numbers like "1", "42", and other "good" numbers have
 * been lording it over the poor minions who are stuck with more boring IDs. To quell the unrest, Commander Lambda
 * has tasked you with reassigning everyone new, random IDs based on her Completely Foolproof Scheme.
 *
 * She's concatenated the prime numbers in a single long string: "2357111317192329...". Now every minion must draw a
 * number from a hat. That number is the starting index in that string of primes, and the minion's new ID number will
 * be the next five digits in the string. So if a minion draws "3", their ID number will be "71113".
 *
 * Help the Commander assign these IDs by writing a function solution(n) which takes in the starting index n of
 * Lambda's string of all primes, and returns the next five digits in the string. Commander Lambda has a lot of
 * minions, so the value of n will always be between 0 and 10000.
 * @author tu.tonquang
 */
public class ReId {
    public static void main(String[] args) {
        System.out.println(solution(0));
        System.out.println(solution(1));
        System.out.println(solution(2));
        System.out.println(solution(3));
        System.out.println(solution(4));
        System.out.println(solution(100));
        System.out.println(solution(1000));
        System.out.println(solution(10000));
    }


    public static String solution(int i) {
        int primeLen = i + 5;
        long prime = 2;
        StringBuilder builder = new StringBuilder();
        builder.append(prime);
        while (builder.length() < primeLen) {
            prime = findPrime(prime);
            builder.append(prime);
        }
        return builder.toString().substring(i, primeLen);
    }

    public static long findPrime(long fromPrime) {
        for (long num = fromPrime + 1; ; num ++) {
            if (isPrime(num)) {
                return num;
            }
        }
    }

    public static boolean isPrime(long num) {
        long sqrt = (long)Math.sqrt(num);
        for (int i = 2; i <= sqrt; i++) {
            if (num % i == 0) {
                return false;
            }
        }
        return true;
    }

}
